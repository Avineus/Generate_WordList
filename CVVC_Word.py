...
document = Document()
document.add_heading('Need for CVVC Word List', 0)

p = document.add_paragraph('CVVC words are useful for kids to start early reading. There are 5 vowels which can be combined with consonants to form words. Words which are formed with consonants +  vowels + vowels + consonants are called CVVC words.Blending words are essential for early readers, this will become easy with familiarity of CVVC words. In this book there are about 116 list of all possible CVVC words. This book has collection of CVVC words having aa, ee and  oo This book has list of CVVC words with its associated part of speech. Table with CVVC words are represented as NOUN, VERB, ADJECTIVE and ADVERB . Flash Card for 335 flash card is also available')
...
for i in v:
   v_count = v_count + 1
   CVVC_Vowel_str = "2 " + "CVVC words Dictionary for vowel " + i 
   document.add_paragraph(CVVC_Vowel_str, style='Heading 1')
   ...
   for j in c:
      ...
      for k in c:
           ...
           if d.check(word) : 
             ...
             if count == 0 :
                CVVC_start_str = "Words having " + i + i 
                document.add_paragraph(CVVC_start_str, style='Heading 2')

                table = document.add_table(rows=1, cols=2)

                table.autofit = True
                table.style = 'Medium Grid 1 Accent 5'

                hdr_cells = table.rows[0].cells
                hdr_cells[0].text = 'CVVC Words'
...
CVVC_Flash_str = "7 Flash Cards for 335 CVVC Words" 
document.add_paragraph(CVVC_Flash_str, style='Heading 1')
   ...
   for j in c:
      for k in c:
           ...
           if d.check(word) : 
              ...
              document.save('Output/CVVC_Word.docx')
